function GmlSpec() {
	...
	#region desired functions
	var _desired_functions = [
		"ds_grid_create",
		"ds_grid_destroy",
		"ds_grid_width",
		"ds_grid_height",
	...
	var _i=0; repeat(array_length(_arr)) {
		...
		if (array_contains(_desired_functions, _elem.Name)) {
			var _index = array_get_index(_desired_functions, _elem.Name)
			var _func_str = _elem.Name;
			var _arg_str = "";
			
			var _str = $"#region jsDoc\n"
			_str += $"/// @func    f{_func_str}()\n"
			_str += $"/// @desc    {_elem.Description}\n"
			
			if (struct_exists(_elem, "Parameter")) {
				var _params = is_struct(_elem.Parameter) ? [_elem.Parameter] : _elem.Parameter;
				var _j=0; repeat(array_length(_params)) {
					var _param = _params[_j];
					_str += $"/// @param   \{{_param.Type}\} {_param.Name} : {_param.Description}\n"
					_arg_str += $"_{_param.Name},"
				_j++}
				if (_j) {
					_arg_str = string_delete(_arg_str, string_length(_arg_str), 1)
				}
			}
			
			_str += $"/// @returns \{{_elem.ReturnType}\}\n"
			_str += $"#endregion\n"
			
			_str += $"function f{_func_str}({_arg_str}) \{\n"
			_str += $"	return {_func_str}({_arg_str})\n"
			_str += $"\}"
			
			_log_arr[_index] = _str;
		}
		//show_debug_message($"\t\t\"{_elem.Name}\": {_elem.Name},")
		
	_i+=1;}//end repeat loop
	
	var _i=0; repeat(array_length(_log_arr)) {
		log(_log_arr[_i])
	_i++}
...
function string_to_constant(_str) {
	static __constantLookup = {
