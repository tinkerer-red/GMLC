...
function ResourceAudioGroupsTestSuite() : TestSuite() constructor {
	...
	addFact("Default audio group test #2", function() {
		...
		assert_equals(loadProgress, 100, "Default audiogroups load progress should be at 100, as it should be loaded in by default");
	...
	addFact("Audiogroup gain test #1", function() {
		...
		// Get gain of audiogroup and test that its 1 by default
	...
	addTestAsync("Audiogroup gain test #3", objTestAsync, {
		...
		ev_create: function() {
			...
			// Set audio groups gain to 0.1 over 0.25 seconds and start a timer
