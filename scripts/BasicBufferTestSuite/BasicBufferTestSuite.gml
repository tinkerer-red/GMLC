...
function BasicBufferTestSuite() : TestSuite() constructor {
	...
	addFact("buffer_poke_test", function() {
		compile_and_execute(@'
		// Set lower epsilon value to ensure rounding errors dont interfere with test results
	...
	addFact("buffer_copy_from_vertex_buffer_test", function() {
		...
		assert_not_equals(output, 1, "buffer_copy_from_vertex_buffer(), failed to copy data into buffer (buffer didnt resize)");
	...
	addFact("buffer_load_test #1", function() {
		...
		assert_equals(output, 512, "buffer_load(), loaded buffer size doesnt match (after buffer_save)");
		output = buffer_peek(loadedBuffer, 256, buffer_u8);
		assert_equals(output, 10, "buffer_load(), loaded buffer data doesnt match (after buffer_save)");
	...
	addFact("buffer_load_test #2", function() {
		...
		assert_equals(output, 256, "buffer_load(), loaded buffer size doesnt match (after buffer_save_ext)");
		output = buffer_peek(loadedBuffer, 0, buffer_u8);
		assert_equals(output, 10, "buffer_load(), loaded buffer data doesnt match (after buffer_save_ext)");
	...
	addFact("buffer_load_ext_test", function() {
		...
		var output = buffer_get_size(loadedBuffer);
		assert_equals(output, 768, "buffer_load_ext(), loaded buffer size doesnt match");
		output = buffer_peek(loadedBuffer, 512, buffer_u8);
		assert_equals(output, 10, "buffer_load_ext(), loaded buffer data doesnt match");
	...
	addFact("buffer_load_partial_test", function() {
		...
		var output = buffer_get_size(loadedBuffer);
		assert_equals(output, 512 + 256, "buffer_load_partial(), loaded buffer size doesnt match");
		output = buffer_peek(loadedBuffer, 512, buffer_u8);
		assert_equals(output, 10, "buffer_load_partial(), loaded buffer data doesnt match");
	...
	addTestAsync("buffer_save_async_test", objTestAsyncSaveLoad, {
		...
		ev_async_save_load: function() {
			compile_and_execute(@'
			// Check if the load event matches the ID returned by buffer_save_async, and if it does, check if its status is true
	...
	addTestAsync("buffer_load_async_test", objTestAsyncSaveLoad, {
		...
		ev_async_save_load: function() {
			compile_and_execute(@'
			// Check if the load event matches the ID returned by buffer_load_async, and if it does, check if its status is true
			var _asyncLoad = async_load;
			if (_asyncLoad[? "id"] != requestID) exit;
