...
/// @description This function will delete the ini file at the provided file path, as well as the directory its located within
...
function BasicIniTestSuite() : TestSuite() constructor {
	...
	addFact("ini_read_real #1", function() {
		...
		// Write a real to the file then read its value to check that they match
		ini_write_real("TestSection", "TestKey", 10);
	...
	addFact("ini_read_string #1", function() {
		...
		// Write a string to the file then read its value to check that they match
	...
	addFact("ini_close #1", function() {
		...
		}, "didnt manage to close the file (can still read/write)");
	...
	addFact("ini_close #2", function() {
		...
		assert_not_equals(_output, "", "#6 ini_close ( ), shouldnt have returned an empty string");
		')
	})
