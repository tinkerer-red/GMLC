...
function BasicFileTestSuite() : TestSuite() constructor {
	...
	addFact("file_test #1", function() {
		...
		// Make sure test directory doesnt exist
	...
	addFact("file_test #7", function() {
		...
		assert_true(value, "file_rename ( oldName, newName ), failed to rename (new file doesnt exist)");
	...
	addFact("file_test #8", function() {
		...
		value = file_exists(firstPath);
		assert_true(value, "file_copy ( oldName, newName ), failed to copy (old file doesnt exist)");
		value = file_exists(copiedPath);
		assert_true(value, "file_copy ( oldName, newName ), failed to rename (new file doesnt exist)");
	...
	addFact("file_bin_test #7", function() {
		...
		}), "file_bin_close ( file ), didnt manage to close the file (can still read/write)");
	...
	addFact("file_text_test #4", function() {
		...
		}), "file_text_close ( file ), didnt manage to close the file (can still read/write)");
	...
	addFact("file_text_open_from_string_test #2", function() {
		...
		assert_false(_output, "file_text_eof (file), shouldnt be at the eof after opening a filled file");
	...
	addFact("filename_test_windows #1", function() {
		...
		assert_equals(fileName, "testFile.txt", "Filename didnt work correctly");
		file_find_close();
	...
	addFact("filename_test_windows #2", function() {
		...
		assert_equals(fileDirectory,expectedDirectory, "filename_dir didnt work correctly");
	...
	addFact("filename_test_windows #5", function() {
		...
		assert_equals(newFilename, filePath + "testFile.doc", "filename_change_ext didnt work correctly");
	...
	addFact("filename_test_macos #1", function() {
		...
		var filePath = filename_path("testFile.txt");
		assert_equals(filePath, localAppDataPath + bundleName + "/", "filename_path didnt work correctly");
	...
	addFact("filename_test_macos #2", function() {
		...
		var fileDirectory = filename_dir("testFile.txt");
		assert_equals(fileDirectory, localAppDataPath + bundleName, "filename_dir didnt work correctly");
