...
/// elements of the `CatspeakGMToken` enum.
///
/// @example
///   Some examples of tokens in Catspeak, and their meanings:
///   - `if`   (is a `CatspeakGMToken.IF`)
///   - `else` (is a `CatspeakGMToken.ELSE`)
///   - `12.3` (is a `CatspeakGMToken.VALUE`)
///   - `+`    (is a `CatspeakGMToken.PLUS`)
enum CatspeakGMToken {
    /// The `(` character.
    PAREN_LEFT,
    /// The `)` character.
    PAREN_RIGHT,
...
function __catspeak_is_token(val) {
    ...
    return is_numeric(val) && (
        val >= 0 && val < CatspeakGMToken.__SIZE__
    );
}
...
function CatspeakLexer(
    ...
    ///
    /// @example
    ///   Prints the string content of the first `CatspeakGMToken` emitted by a
    ///   lexer.
    ///
    ///   ```gml
    ///   lexer.next();
    ...
    static __getKeyword = function (str) {
        ...
        if (CATSPEAK_DEBUG_MODE && keyword != undefined) {
            __catspeak_check_arg(
                    "keyword", keyword, __catspeak_is_token, "CatspeakGMToken");
        }
        return keyword;
    };
    ...
    /// Advances the lexer and returns the next type of `CatspeakGMToken`. This
    ...
    ///     if (token != CatspeakGMToken.WHITESPACE) {
    ///       show_debug_message(lexer.getLexeme());
    ///     }
    ///   } until (token == CatspeakGMToken.EOF);
    ///   ```
    ///
    /// @return {Enum.CatspeakGMToken}
    static nextWithWhitespace = function () {
        __clearLexeme();
        if (charNext == 0) {
            return CatspeakGMToken.EOF;
        }
        __advance();
        var token = CatspeakGMToken.OTHER;
        ...
        if (
            ...
            if (isRaw) {
                token = CatspeakGMToken.VALUE; // since `@` is an operator
                __advance();
            }
            ...
            if (keyword != undefined) {
                token = keyword;
                if (keyword == CatspeakGMToken.COMMENT) {
                    // consume the comment
        ...
        } else if (token == CatspeakGMToken.IDENT) {
            ...
            if (digitCount == 3) {
                ...
                // invalid
                token = CatspeakGMToken.OTHER;
            }
            ds_stack_destroy(digitStack);
            value = cR | (cG << 8) | (cB << 16) | (cA << 24);
            hasValue = true;
        } else if (token == CatspeakGMToken.VALUE) {
            // numeric literals
    ...
    /// Advances the lexer and returns the next `CatspeakGMToken`, ignoring any
    ...
    ///   ```gml
    ///   var lexer = new CatspeakLexer(buff);
    ///   while (lexer.peek() != CatspeakGMToken.EOF) {
    ///     lexer.next();
    ///     show_debug_message(lexer.getLexeme());
    ///   }
    ///   ```
...
function __catspeak_init_lexer_codepage() {
    var page = array_create(__CATSPEAK_CODEPAGE_SIZE, CatspeakGMToken.OTHER);
    for (var code = 0; code < __CATSPEAK_CODEPAGE_SIZE; code += 1) {
        ...
        if (__catspeak_codepage_set(code,
            ...
            tokenType = CatspeakGMToken.WHITESPACE;
        } else if (
            __catspeak_codepage_range(code, "a", "z") ||
            __catspeak_codepage_range(code, "A", "Z") ||
            __catspeak_codepage_set(code, "_", "`") // identifier literals
        ) {
            tokenType = CatspeakGMToken.IDENT;
        } else if (
            __catspeak_codepage_range(code, "0", "9") ||
            __catspeak_codepage_set(code, "'") // character literals
        ) {
            tokenType = CatspeakGMToken.VALUE;
        } else if (__catspeak_codepage_set(code, "\"")) {
            tokenType = CatspeakGMToken.VALUE;
        } else if (__catspeak_codepage_set(code, "(")) {
            tokenType = CatspeakGMToken.PAREN_LEFT;
        } else if (__catspeak_codepage_set(code, ")")) {
            tokenType = CatspeakGMToken.PAREN_RIGHT;
        } else if (__catspeak_codepage_set(code, "[")) {
            tokenType = CatspeakGMToken.BOX_LEFT;
        } else if (__catspeak_codepage_set(code, "]")) {
            tokenType = CatspeakGMToken.BOX_RIGHT;
        } else if (__catspeak_codepage_set(code, "{")) {
            tokenType = CatspeakGMToken.BRACE_LEFT;
        } else if (__catspeak_codepage_set(code, "}")) {
            tokenType = CatspeakGMToken.BRACE_RIGHT;
        } else if (__catspeak_codepage_set(code, ",")) {
            tokenType = CatspeakGMToken.COMMA;
        } else {
            continue;
        }
        page[@ code] = tokenType;
...
function __catspeak_keywords_create() {
    var keywords = { };
    keywords[$ "--"] = CatspeakGMToken.COMMENT;
    keywords[$ ";"] = CatspeakGMToken.SEMICOLON;
    keywords[$ "..."] = CatspeakGMToken.WHITESPACE; // legacy
    keywords[$ ":"] = CatspeakGMToken.COLON;
    keywords[$ ","] = CatspeakGMToken.COMMA;
    keywords[$ "."] = CatspeakGMToken.DOT;
    keywords[$ "="] = CatspeakGMToken.ASSIGN;
    keywords[$ "*="] = CatspeakGMToken.ASSIGN_MULTIPLY;
    keywords[$ "/="] = CatspeakGMToken.ASSIGN_DIVIDE;
    keywords[$ "-="] = CatspeakGMToken.ASSIGN_SUBTRACT;
    keywords[$ "+="] = CatspeakGMToken.ASSIGN_PLUS;
    keywords[$ "%"] = CatspeakGMToken.REMAINDER;
    keywords[$ "*"] = CatspeakGMToken.MULTIPLY;
    keywords[$ "/"] = CatspeakGMToken.DIVIDE;
    keywords[$ "//"] = CatspeakGMToken.DIVIDE_INT;
    keywords[$ "-"] = CatspeakGMToken.SUBTRACT;
    keywords[$ "+"] = CatspeakGMToken.PLUS;
    keywords[$ "=="] = CatspeakGMToken.EQUAL;
    keywords[$ "!="] = CatspeakGMToken.NOT_EQUAL;
    keywords[$ ">"] = CatspeakGMToken.GREATER;
    keywords[$ ">="] = CatspeakGMToken.GREATER_EQUAL;
    keywords[$ "<"] = CatspeakGMToken.LESS;
    keywords[$ "<="] = CatspeakGMToken.LESS_EQUAL;
    keywords[$ "!"] = CatspeakGMToken.NOT;
    keywords[$ "~"] = CatspeakGMToken.BITWISE_NOT;
    keywords[$ ">>"] = CatspeakGMToken.SHIFT_RIGHT;
    keywords[$ "<<"] = CatspeakGMToken.SHIFT_LEFT;
    keywords[$ "&"] = CatspeakGMToken.BITWISE_AND;
    keywords[$ "^"] = CatspeakGMToken.BITWISE_XOR;
    keywords[$ "|"] = CatspeakGMToken.BITWISE_OR;
    keywords[$ "and"] = CatspeakGMToken.AND;
    keywords[$ "or"] = CatspeakGMToken.OR;
    keywords[$ "xor"] = CatspeakGMToken.XOR;
    keywords[$ "|>"] = CatspeakGMToken.PIPE_RIGHT;
    keywords[$ "<|"] = CatspeakGMToken.PIPE_LEFT;
    keywords[$ "do"] = CatspeakGMToken.DO;
    keywords[$ "if"] = CatspeakGMToken.IF;
    keywords[$ "else"] = CatspeakGMToken.ELSE;
    keywords[$ "while"] = CatspeakGMToken.WHILE;
    keywords[$ "for"] = CatspeakGMToken.FOR;
    keywords[$ "loop"] = CatspeakGMToken.LOOP;
    keywords[$ "match"] = CatspeakGMToken.MATCH;
    keywords[$ "use"] = CatspeakGMToken.USE;
    keywords[$ "let"] = CatspeakGMToken.LET;
    keywords[$ "fun"] = CatspeakGMToken.FUN;
    keywords[$ "params"] = CatspeakGMToken.PARAMS;
    keywords[$ "break"] = CatspeakGMToken.BREAK;
    keywords[$ "continue"] = CatspeakGMToken.CONTINUE;
    keywords[$ "return"] = CatspeakGMToken.RETURN;
    keywords[$ "new"] = CatspeakGMToken.NEW;
    keywords[$ "impl"] = CatspeakGMToken.IMPL;
    keywords[$ "self"] = CatspeakGMToken.SELF;
    return keywords;
}
...
///
/// @param {Struct} keywords
/// @param {Enum.CatspeakGMToken} token
///
/// @return {String}
function __catspeak_keywords_find_name(keywords, token) {
    if (CATSPEAK_DEBUG_MODE) {
        __catspeak_check_arg("keywords", keywords, is_struct);
        __catspeak_check_arg(
                "token", token, __catspeak_is_token, "CatspeakGMToken");
    }
