	...
	*/
	...
	function GMLC_Gen_4_Optimizer(_env) constructor  {
		...
		static constantPropagation = function(_node_data) {
			...
			if (_found) {
				...
				var _i = 0; repeat(array_length(_children))
				{
					...
					if (_child == _node) {
						show_debug_message($"Optimizer :: constantPropagation :: Has found Constant `{_constant_data.identifier}` in line ({_node.line}) `{_node.lineString}`")
		...
		static __propagateConstants = function(_node, _constant_data) {
			//show_debug_message($"__propagateConstants :: typeof(_node) == {typeof(_node)} :: instanceof(_node) == {instanceof(_node)}")
		...
		static __propagateToChildren = function(_node, _constant_data) {
			//show_debug_message($"__propagateToChildren :: typeof(_node) == {typeof(_node)} :: instanceof(_node) == {instanceof(_node)}")
					...
					//single literal
					    ...
					    switch (_node.operator) {
							case "+":{
								...
								&& (_node.left.value == 0) {
									show_debug_message($"Optimizer :: constantFolding :: Could remove addtion of `0` in line ({_node.line}) `{_node.lineString}`")
									return _node.right;
								}
							...
							case "-":{
								...
								if (_node.left.type  == __GMLC_NodeType_Literal)
								&& (_node.left.value == 0) {
									show_debug_message($"Optimizer :: constantFolding :: Could remove subtraction of `0` in line ({_node.line}) `{_node.lineString}`")
									return _node.right;
								}
								if (_node.right.type  == __GMLC_NodeType_Literal)
								&& (_node.right.value == 0) {
							...
							case "*":{
								...
								if (_node.left.type  == __GMLC_NodeType_Literal)
								&& (_node.left.value == 1) {
									show_debug_message($"Optimizer :: constantFolding :: Could remove unneeded multiplication of `1` in line ({_node.line}) `{_node.lineString}`")
									return _node.right;
								}
								if (_node.right.type  == __GMLC_NodeType_Literal)
								&& (_node.right.value == 1) {
							...
							case "/":{
								...
								if (_node.right.type  == __GMLC_NodeType_Literal)
								&& (_node.right.value == 1) {
									show_debug_message($"Optimizer :: constantFolding :: Could remove unneeded division of `1` in line ({_node.line}) `{_node.lineString}`")
									return _node.left;
								}
				...
				case __GMLC_NodeType_NullishExpression:{
					if (_node.left.type == __GMLC_NodeType_Literal) {
						if (_node.left.value == undefined) {
							show_debug_message($"Optimizer :: constantFolding :: Could collapse nullish express to right side only in line ({_node.line}) `{_node.lineString}`")
							return _node.right;
						}
						else {
							show_debug_message($"Optimizer :: constantFolding :: Could collapse nullish express to left side only in line ({_node.line}) `{_node.lineString}`")
							return _node.left;
						}
					}
				break;}
				...
				case __GMLC_NodeType_ConditionalExpression:{
					if (_node.condition.type == __GMLC_NodeType_Literal) {
						...
						if (_node.condition.value) {
							show_debug_message($"Optimizer :: constantFolding :: Could collapse ternary expression to right side in line ({_node.line}) `{_node.lineString}`")
		...
		static eliminateDeadCode = function(_node_data) {
			...
			switch (_node.type) {
				case __GMLC_NodeType_IfStatement:{
					if (_node.condition.type == __GMLC_NodeType_Literal) {
						if (_node.condition.value) {
							show_debug_message($"Optimizer :: eliminateDeadCode :: Could optimize `if` statement to `true` block only in line ({_node.line}) `{_node.lineString}`")
							return _node.consequent;
						}
						else {
							if (_node.alternate != undefined) {
								show_debug_message($"Optimizer :: eliminateDeadCode :: Could optimize `if` statement to `else` block only in line ({_node.line}) `{_node.lineString}`")
				...
				case __GMLC_NodeType_ConditionalExpression:{
					if (_node.condition.type == __GMLC_NodeType_Literal) {
					    // If the condition is a literal, determine which branch to take
						show_debug_message($"Optimizer :: eliminateDeadCode :: Could optimize ternary expression given a condition of {_node.condition.value} in line ({_node.line}) `{_node.lineString}`")
