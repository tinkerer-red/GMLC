...
function start_draw_comparison(_width = room_width, _height = room_height) {
	...
	{
		// End the comparison here if not, since we dont have a surface to work with
		assert_true(false, test_current().name + ", failed to create a surface for comparison");
		// Return undefined to tell end_draw_comparison that the test surface wasnt created
		return undefined;
	}
...
function end_draw_comparison(_test_surface, _test_name, _fail_message) {
	...
	if (!surface_exists(_test_surface))
	{
		// End the comparison here if so, since we dont have a surface to work with
	...
	if (_test_name == "")
	{
		// Delete the surface and end the comparison here if so, since we dont have a file name to save to/load from
	...
	{
		// End the comparison here if not, since we dont have a buffer to work with
	...
	{
		// check the test buffer against the expected buffer (showing the fail message in the assert if they dont match)
