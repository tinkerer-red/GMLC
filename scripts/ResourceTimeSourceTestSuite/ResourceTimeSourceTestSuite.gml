...
function generate_tree(_numSources, _builtInRoot = true) 
{
	...
	// This isnt necessarily a built-in (though non built-ins will still have a built-in parent)
	if (_builtInRoot)
	{
		_root = {
...
function ResourceTimeSourceTestSuite() : TestSuite() constructor {
	...
	var _seed = randomise();
	do_trace("Using seed: " + string(_seed));
	...
	addFact("Time Sources: Existence", function() {
		...
		_output = time_source_exists(time_source_global);
		assert_true(_output, "#3 time_source_destroy(), destroyed built-in source: time_source_global (shouldnt)");
			
		time_source_destroy(time_source_game);
		_output = time_source_exists(time_source_game);
		assert_true(_output, "#4 time_source_destroy(), destroyed built-in source: time_source_game (shouldnt)");
		...
		time_source_destroy(_ts1); // Cant destroy if it has children
		_output = time_source_exists(_ts1);
		assert_true(_output, "#9 time_source_destroy(), destroyed a source with children (shouldnt)");
	...
	addFact("Time Sources: State Transitions", function() {				
		...
		// Checks a time sources and all of its descendants states
	...
	addFact("Time Sources: Parents and Children", function() {
		...
		// Check that each sources parent and children are what we expect
	...
	addTestAsync("Time Sources: Expiry Frames", objTestAsync, {
		...
		ev_create: function() {
			...
			// As we dont have access to the microsecond timer resolution here as well as the true frame-to-frame time deltas,
			...
			function generateRandomConfigWithExpiryFrame(_gameSpeed, _maxPeriodSecs, _maxPeriodFrames, _maxRepeats) constructor
			{
				...
				if (units == time_source_units_seconds)
				{
					...
					// Set a minimum period of a frames period
					...
					if (expiryType == time_source_expire_nearest)
					{
						...
						// Cant use the round function because it uses bankers rounding...
		...
		ev_step: function() {
			...
			for (var _i = 0; _i < numTests; ++_i)
			{
				...
				// If were not in the sources tolerance zone (only applies to sources measured in seconds)
	...
	addTestAsync("Time Sources: Time delta propagation", objTestAsync, {
		...
		// and then make sure that the remaining time isnt decremented for that new game source.
		ev_create: function() {
			compile_and_execute(@'
			...
			var _callback = function() {
				// We shouldnt see this execute unless time deltas propagated after the first callback was invoked
