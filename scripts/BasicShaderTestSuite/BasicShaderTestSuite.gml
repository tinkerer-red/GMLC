...
function verify_shader_compiled(_shader) {
	...
	if (!_is_compiled)
	{
		// End test early if it hasnt, as the test will not run correctly
		test_end();
	}
}
...
function BasicShaderTestSuite() : TestSuite() constructor {
	...
	addTestAsync("shader_is_compiled", objTestAsyncDraw, {
		...
		ev_create: function() {
			...
			for (i = 0; i < _shader_count; i++)
			{
				...
				//do_trace(shaders[i]); // Uncomment this to check which shaders have and havent been considered compiled by shader_is_compiled()
			}
			')
		},
		ev_draw: function() {
			...
			for (i = 0; i < array_length(shaders); i++)
			{
				...
				{
					// Check that the function doesnt throw an exception when set, confirming that it has been compiled
	...
	addFact("shaders_are_supported", function() {
		...
		if (_output == false)
		{
			...
			else if (os_type == os_android)
			{
				...
				// If info map contains an entry with information about the platforms supported shader language..
				if (ds_map_exists(_info, "GL_SHADING_LANGUAGE_VERSION"))
				{
					...
					{
						// Check that the shader language name doesnt have "GLSL ES" in it (because if it uses any version of GLSL ES then shaders should be supported)
	...
	addTestAsync("shader_current", objTestAsyncDraw, {
		...
		ev_draw: function() {
			...
			shader_set(test_shader);
				// Test that shader_current() returns the shader were currently using
	...
	addTestAsync("shader_get/set_uniform_f", objTestAsyncDraw, {
		...
		ev_create: function() {
			...
			// Stores which frame of the draw event were on
			draw_frame = 0;
			')
		},
		ev_draw: function() {
			...
			// Set test variables based on which draw frame were on
			switch (draw_frame)
			{
	...
	addTestAsync("shader_enable_corner_id", objTestAsyncDraw, {
		...
		ev_draw: function() {
			...
			// Start using shader
			shader_set(test_shader);
				// Draw a sprite (a sprite is used instead of a rectangle because drawn primitives cant have corner ids)
