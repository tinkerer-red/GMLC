...
if(!ds_exists(map,ds_type_map)) {
    ...
    draw_rectangle(view_xview[0], 0, view_xview[0]+view_wview[0], (room_height*0.6), 0)
    for(var i=(view_xview[0]-view_wview[0]/2)-(view_xview[0]%128); i<view_xview[0]+view_wview[0]*3/2; i+=128) {        
    ...
    if(data[?"game state"]=gamestate.play || data[?"game state"]=gamestate.death) {
        ...
        for(var n=0; n<ds_list_size(data[?"enemies"]); n++) {
            ...
            if((!ds_map_exists(currentEnemy,"death") || currentEnemy[?"death"]>data[?"game step"]) && currentEnemy[?"y"]<room_height+64 && currentEnemy[?"x"]>view_xview[0]-64 && currentEnemy[?"x"]<view_xview[0]+view_wview[0]+64) {
                var color=$777777;
        ...
        if(!surface_exists(data[?"surface ghosts"])) {
            data[?"surface ghosts"]=surface_create(view_wview[0],view_hview[0])
        }
        ...
        for(var n=0; n<ds_list_size(data[?"prev runs"]); n++) {
            ...
            if(currentGhost[?"y"]<room_height+64 && currentGhost[?"x"]>view_xview[0]-64 && currentGhost[?"x"]<view_xview[0]+view_wview[0]+64) {
                if(sprite_exists(data[?"surface bird"])) {
                    draw_sprite_ext(data[?"surface bird"], 0, round(currentGhost[?"x"]-view_xview[0]), round(currentGhost[?"y"]-view_yview[0]), 1, 1, sin(currentGhost[?"x"]/30)*10-angle, $777777, 1) 
                }
            }
        ...
        surface_reset_target()
        draw_surface_ext(data[?"surface ghosts"],view_xview[0],view_yview[0],1,1,0,c_white,0.1)
    ...
    if(data[?"game state"]=gamestate.play) {        
        ...
        view_xview[0]=floor(lerp(view_xview[0], myBird[?"x"]-view_wview[0]*0.2, 0.1))
        ...
        if(ds_list_empty(enemies)) {
            ...
            enemy[?"x"]=max(view_xview[0],0)+view_wview[0]
            enemy[?"y"]=view_hview[0]-irandom(view_hview[0]/3)
        ...
        if(lastEnemy[?"x"]<max(view_xview[0],0)+view_wview[0]+enemiesDistance) {
            var used=false;
    ...
    if(data[?"game state"]=gamestate.death) {
        ...
        //"YOU DIED!" text
        var _x=view_xview[0]+view_wview[0]*0.5;
        var _y=view_yview[0]+view_hview[0]*0.4;
        var _a=0;       
        ...
        //"Press SPACE" text
        var _x=view_xview[0]+view_wview[0]*0.5;
        var _y=view_yview[0]+view_hview[0]*0.6;
        var _a=0;
    ...
    if(data[?"game state"]=gamestate.menu) {
        ...
        view_xview[0]=floor(lerp(view_xview[0], -view_wview[0], 0.1))
        ...
        //"FLAPPY SOULS" text
        var _x=view_xview[0]+view_wview[0]*0.5;
        var _y=view_yview[0]+view_hview[0]*0.25;
        var _a=0;       
        ...
        var _x=view_xview[0]+view_wview[0]*0.5;
        var _y=view_yview[0]+view_hview[0]*0.85;
        var _a=0;
