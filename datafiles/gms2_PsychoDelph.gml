...
function PsychoDelph() {
	...
	*/
	...
	__background_set_colour( make_colour_hsv(irandom(255) + 255 * _msec * 0.000005 , irandom(150)+80, irandom(63)+160) );
	...
	if application_surface_is_enabled() {
	  ...
	  if not background_exists(__background_get( e__BG.Index, 0 )) {
	    __background_set( e__BG.Index, 0, background_create_from_surface(_surf, 0, 0, 16*_img_count, 16, false, false) )
	  }   
	  if not background_exists(__background_get( e__BG.Index, 1 )) {
	    __background_set( e__BG.Index, 1, background_create_colour(32, 32, c_white) )  // img_build
	  }   
 
	  surface_free(_surf)

	  // target
	  var target = tile_add(__background_get( e__BG.Index, 0 ), 16, 0, 16, 16, 0, 0, target_depth)
	...
	var img_enemy = 0;
	img_enemy[_i.index]  = __background_get( e__BG.Index, 0 )
	img_enemy[_i.x] = 0
	img_enemy[_i.y] = 0
	...
	var img_build = 0;
	img_build[_i.index]  = __background_get( e__BG.Index, 1 )
	img_build[_i.x] = 0
	img_build[_i.y] = 0
	...
	for (var i=0; i<build_layer_count; i++) {
	  ...
	  build_layer[i, _bl.color] = make_colour_hsv(
	    colour_get_hue(__background_get_colour( )),
	    colour_get_saturation(__background_get_colour( )),
	    colour_get_value(__background_get_colour( ))/(build_layer_count+1) * (build_layer_count-i)
	  ) 
	...
	randomize()
	if random(1)<_rate {
	  __background_set_colour( make_colour_hsv(
	    colour_get_hue(__background_get_colour( )),
	    colour_get_saturation(__background_get_colour( )),
	    255
	  ) )
	...
	if _show_debug {
	  ...
	  draw_text(50, 20, string_hash_to_newline(string(fps_real)))
	  draw_text(50, 50,   string_hash_to_newline("Hue: " + string(colour_get_hue(__background_get_colour( ))) +
	                    "; Sat: " + string(colour_get_saturation(__background_get_colour( ))) +
	                    "; Val: " + string(colour_get_value(__background_get_colour( ))))
	  )
	  draw_text(50, 80,  string_hash_to_newline("rate: "    + string(_rate*100)+"%"))
	  draw_text(50, 110, string_hash_to_newline("spawned: " + string(enemy_count)))
	  draw_text(50, 140, string_hash_to_newline("mouse affect:"))
	  draw_text(50, 170, string_hash_to_newline("     x: " + string(_dmx)))
	  draw_text(50, 200, string_hash_to_newline("     y: " + string(_dmy)))
	}
