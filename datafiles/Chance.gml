...
if (!ds_exists(0,ds_type_map))
  { randomize();
    ...
    background_colour = c_dkgray; background_showcolour = true;
    ...
    for (var i=0; i<3; i++;)
      { draw_clear_alpha(c_black,0);
        ...
        if (surface_exists(surf_bk))
             { background_index[i] = background_create_from_surface(surf_bk,0,0,W,H,0,0); }
        else { background_index[i] = background_create_colour(W,H,c2); }
      }
        ...
        { if (n == 1) { w = 12; w0 = 120; } // last minute kludge for second text
          ...
          for (var m=1; m<string_length(text[n])+1; m++;)  // write character to small surface
            { char = string_copy(text[n],m,1);
              ...
              draw_clear_alpha(c_black,0); draw_text(w/2,h/2,char);
    ...
    { var L0 = 400;  
      ...
      if ( !game_over )
        { for (var i=1; i<11; i++;) // 11 images in sprite
          ...
          draw_text_transformed(W/2,L0-200,"Press SPACE to...",1.5,1.5,0);
          draw_text_transformed(x0,y0,"JUMP",1.8,1.8,rot);
          draw_text_transformed(W/2,L0+25,"Press SPACE to Start Game",1.5,1.5,0);
        }
      else
        { for (var i=1; i<10; i++;) // 10 images in sprite
            { draw_sprite(spr_win,i,90*i,100+20*sin(i*pi/4+count)); } 
          var start_key = ord("R");
          draw_text_transformed(W/2,L0-200,"Vote for...",1.5,1.5,0);
          draw_text_transformed(x0,y0,"CHANCE",1.8,1.8,rot);
          draw_text_transformed(W/2,L0+25,"Press R to Replay",1.5,1.5,0);
          draw_set_color(c_red);
          draw_text_transformed(W/2,L0-50,"You Had "+string(hits)+" Misses",1.5,1.5,0);
        }
    ...
    { var L1 = 125; var L2 = 290; var L3 = 465; // level heights
      ...
      switch(level) // Switch stucture chosen over data structure for ease of Level editing
        { case 1: p1 = 0.5*W; p2 = 5*W; p3 = 7*W; // block locations
            ...
            draw_text_transformed(W/2,grnd+25,"Press SPACE to Jump Block",1.5,1.5,0);
          break;
          ...
          case 2: p1 = 0.4*W; p2 = 0.6*W; p3 = 8*W;
            ...
            draw_text_transformed(W/2,grnd+25,"OK, a Little Harder Now.",1.5,1.5,0);
          break;
          ...
          case 3: p1 = 0.3*W; p2 = 0.5*W; p3 = 0.7*W;
            ...
            spr_obs = spr_spike;  
            start = -2*d; stop = W; theta = -10;
            draw_text_transformed(W/2,grnd+25,"Watch for Spikes..!",1.5,1.5,0);
          break;
          ...
          case 4: p1 = 0.3*W; p2 = 0.5*W; p3 = 0.7*W;
            ...
            spr_obs = spr_block;
            start = W+2*d; stop = 0; theta = 10;
            background_visible[0] = false; background_visible[1] = true;
            draw_text_transformed(W/2,grnd+25,"Getting a Bit More Serious.",1.5,1.5,0);
          break;
          ...
          case 5: p1 = 0.35*W; p2 = 0.5*W; p3 = 0.7*W;
            ...
            spr_obs = spr_spike;
            start = -2*d; stop = W; theta = -10;
            draw_text_transformed(W/2,grnd+25,"No, Really...",1.5,1.5,0);
          break;
          ...
          case 6: p1 = 0.20*W; p2 = 0.45*W; p3 = 0.6*W;
            ...
            spr_obs = spr_block;
            start = W+2*d; stop = 0; theta = 10;
            draw_text_transformed(W/2,grnd+25,"Timing!  Timing!",1.5,1.5,0);
          break;
          ...
          case 7: p1 = 0.2*W; p2 = 0.45*W; p3 = 0.7*W;
            ...
            spr_obs = spr_block;
            start = -2*d; stop = W; theta = -10;
            background_visible[1] = false; background_visible[2] = true;
            draw_text_transformed(W/2,grnd+25,"Three Levels Remaining.",1.5,1.5,0);
          break;
          ...
          case 8: p1 = 0.35*W; p2 = 0.55*W; p3 = 0.7*W;
            ...
            spr_obs = spr_spike;
            start = W+2*d; stop = 0; theta = 10;
            draw_text_transformed(W/2,grnd+25,"Almost Done.",1.5,1.5,0);
          break;
          ...
          case 9: p1 = 0.2*W; p2 = 0.45*W; p3 = 0.6*W;
            ...
            spr_obs = spr_block;
            start = -2*d; stop = W; theta = -10;
            draw_text_transformed(W/2,grnd+25,"The Final Level.",1.5,1.5,0);
          break;
...
// RENDER EVERYTHING
     ...
     draw_text_transformed(80,20,"Missed: "+string(hits),1.3,1.3,0); // Stats Display
     draw_text_transformed(W-100,20,"Level: "+string(level)+" of 9",1.3,1.3,0);
